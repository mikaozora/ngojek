...
<body>
    <?php
    $sql = "SELECT og.destination, og.total, g.name, og.order_goodsid from order_goods og
    JOIN goods g ON og.goods_id = g.goods_id where og.customer_id = '$logged_customer' limit 0, 7";
    $res = mysqli_query($conn, $sql);
    while ($row = mysqli_fetch_assoc($res)) :
    ?>
        <div>
            <a class="wrap-content" href="transactionDetail_ride&sent.php?id=<?=$row["order_goodsid"]?>&type=goods" style="color: black; text-decoration:none; margin:10px 0;">
