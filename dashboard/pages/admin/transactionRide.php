...
$rows = mysqli_query($conn, "SELECT od.order_driverid, od.destination, od.source, od.transaction_date, od.total ,d.driver_id, d.vehicle, d.name AS driver_name, d.number_plate, c.customer_id, c.name AS customer_name FROM order_driver od
JOIN driver d ON od.driver_id = d.driver_id
...
<body>
    <table class="table table-bordered bg-white">
        ...
        <tbody>
            <?php
            $no = 1;
            $sql = "SELECT od.order_driverid, od.destination, od.source, od.transaction_date, od.total ,d.driver_id, d.vehicle, d.name AS driver_name, d.number_plate, c.customer_id, c.name AS customer_name FROM order_driver od
            JOIN driver d ON od.driver_id = d.driver_id
            JOIN customers c ON od.customer_id = c.customer_id limit $start, $rowPerPage";
                    ...
                    <div class="modal-dialog wrap-detail">
                        <div class="modal-content">
                            <div class="modal-header">
                                <h5 class="modal-title" id="staticBackdropLabel">Detail Transaction Ride</h5>
                                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                            </div>

                            <!-- <input type="hidden" name="order_driverid" value="<?= $row["order_driverid"] ?>"> -->
                            <div class="modal-body">
                                <div class="wrap-header-detail">
                                    <h3>Source</h3>
                                    <h3>Destination</h3>
                                    <h3>Number Plate</h3>
                                    <h3>Vehicle</h3>
