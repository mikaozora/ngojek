...
$rows = mysqli_query($conn, "SELECT * from order_goods");
...
<body>
    <table class="table table-bordered bg-white">
        ...
        <tbody>
            <?php
            $no = 1;
            $sql = "SELECT og.order_goodsid, og.total ,og.transaction_date, og.source, og.destination,d.driver_id, d.vehicle, d.name AS driver_name, d.number_plate, c.customer_id, c.name AS customer_name, g.name AS goods_name, g.weight, g.dimension, g.description
            FROM order_goods og
                        JOIN driver d ON og.driver_id = d.driver_id
                        JOIN customers c ON og.customer_id = c.customer_id
                        JOIN goods g ON og.goods_id = g.goods_id limit $start, $rowPerPage";
